...
class LogisticRegressor:
    ...
    def loss(self, *args):
        ...
        J = (1./m) * (-np.dot(y.T, np.log(g)) - np.dot((1.-y.T),np.log(1. - g)))
...
class RegLogisticRegressor:
    ...
    def loss(self, *args):
        ...
        J = (1./m) * (-np.dot(y.T, np.log(g)) - np.dot((1.-y.T),np.log(1. - g))) + (reg/(2.*m)) *np.dot(theta[1:].T,theta[1:])
    ...
    def grad_loss(self, *args):
        ...
        grad = (1. / m) * (np.dot(X.T,(g - y.T))) + (reg/m)*thetaj

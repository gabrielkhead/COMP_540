...
class RegularizedLinearReg_SquaredLoss(RegularizedLinearRegressor_Multi):
    ...
    def loss (self,*args):
        ...
        num2 = (np.dot(X, theta) - y)**2
        #num2 = np.power(numerator, 2)
        J = np.sum(num2) / (2.0 * len(X)) + np.matmul(theta[1:],theta[1:].T) * reg / (2.0 * len(X))
    ...
    def grad_loss(self,*args):                                                                          
        ...
        theta_j = theta
        theta_j[0]=0.
        grad = (1.0 / len(X)) * np.dot(X.T, numerator) + reg *theta_j  / len(X)

...
def image_to_numpy(path):
    ...
    for dirs in os.walk(path):
        for myFile in dirs[1]:
            path1 = path+'\\'+myFile
            for subdirs in os.walk(path1):
                if 'images' in subdirs[0]:
                    file = glob.glob(subdirs[0]+'/*.png')
                    ...
                    indx += 1
                    if indx%50 ==0:
                        print('{} images processed'.format(indx))
                elif 'masks' in subdirs[0]:
                    file = glob.glob(subdirs[0]+'/*.png')
                    temp = np.zeros(cv2.imread(file[0]).shape)
                    for i in range(0,len(file)):
                        mask = cv2.imread(file[i])
                        temp = mask + temp
                        #COMBINE ALL MASKS
                    X_masks.append(temp)
    X_img_arrays = np.asarray(X_images)
    X_mask_arrays  = np.asarray(X_masks)
    print('There are {} unique image sizes'.format(len(set(sizes.values()))))
    print(set(sizes.values()))
    return(X_img_arrays, X_mask_arrays, sizes)
